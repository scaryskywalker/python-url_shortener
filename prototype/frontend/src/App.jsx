...
function App() {
  ...
  const validateAndLoadDashboard = async (key, silent = false) => {
    try {
      ...
      setAuthLoading(true);
      setAuthError('');
      await loadData(cleanKey);
      setApiKey(cleanKey);
      localStorage.setItem('merchantKey', cleanKey);
      setIsAuthenticated(true);
    } catch (err) {
      if (err.response?.status === 401 || err.response?.status === 403) {
        handleLogout();
        if (!silent) setAuthError(err.response.data?.detail || 'Invalid or inactive API key.');
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const generatedKey = res.data.api_key;
      setNewApiKey(generatedKey);
      setShowKeyModal(true);
      await validateAndLoadDashboard(generatedKey);
    } catch (err) {
      ...
      {showKeyModal && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <h2 className="modal-title">Account Provisioned</h2>
            <p className="modal-description">
              Please copy and save this API key now. It will not be shown again.
            </p>
            <div className="api-key-box">
              <code className="api-key-text">{newApiKey}</code>
              <button 
              ...
              >
                {keyCopied ? <CheckCircle2 size={20} color="var(--success)" /> : <Copy size={20} />}
              </button>
            </div>
            <button 
